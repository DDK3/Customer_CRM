...
public class ServiceScheduleAdapter extends RecyclerView.Adapter<ServiceScheduleAdapter.ViewHolder> {
    ...
    public ServiceScheduleAdapter(Context mContext, List<Settergetter> worldpopulationlist) {
    ...
    @Override
    public ServiceScheduleAdapter.ViewHolder onCreateViewHolder(ViewGroup parent, int viewType) {
        View view = LayoutInflater.from(mContext).inflate(R.layout.service_schedule_list,parent,false);
        return new ViewHolder(view);
    }

    @Override
    public void onBindViewHolder(ServiceScheduleAdapter.ViewHolder holder, int position) {

        Settergetter list = worldpopulationlist.get(position);
        holder.machine_name.setText(list.getCust_name());
...
//            this.title = (TextView) itemView.findViewById(R.id.ac_name_edit);
            this.machine_name = (TextView) itemView.findViewById(R.id.ac_name_edit);
...
//            mContext.startActivity(new Intent(mContext, ServiceMachineList.class));
            int position= getAdapterPosition();
            Settergetter list = arrayListimage.get(position);
            String title1 = list.getCust_id();
            Intent intent = new Intent(mContext,ServiceMachineList.class);
